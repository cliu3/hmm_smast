...
fvcom_tidaldb = 'data/fvcomdb_gom4_NECOFS.mat';
global bottom_temperature  % path to fvcom bottom temperature
bottom_temperature   = 'data/gom4_btemp_davged_NECOFS_2017_20d.nc';

path_to_tags = 'R_HMM_Preprocessing/processed_tags/' ;

D = [1., 10.]; % Diffusivity in km^2/day

global std_temp_offset std_depth_offset tag_depth_range tag_depth_accu tag_temp_accu low_fit mod_fit
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% tag-specific paremeters  %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
std_temp_offset=2.0; %higher value is more inclusive
std_depth_offset=20.0;
...
for tag_num=tag_num_range
    ...
    do_parts(5) = 1; %5 cliu likelihood & tidal constraint
        do_tidal_exclusion = 1;
        if ismember(tag_num, [4, 5, 6])
          do_tidal_exclusion = 0;
        end
    ...
    if(do_parts(1)==1)
        ...
        gen_tidaldb_draft(-71,-66,42.4,44.2,.05); %GoM
    ...
    if(exist([path_to_tags tagname '.mat']))
        ...
        if(do_parts(5)==1)
            ...
            if(do_tidal_exclusion == 1)
                tidal_rmse_cliu(tag_num,path_to_tags,tagname)
            else
                interp_regular(tag_num,path_to_tags)
            end
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%% Geolocate the tag                                              %%%
        ...
        if(do_parts(6)==1)
            ...
            %hmmgeolocate(tagid,2,'on',[1.,10.]); %1,2
            hmmgeolocate(tagid,2,'on',D); %3,4
        end;
