function plotting(day,result,td,cmap,rate,mpt,zm)
%PLOTTING Plot the result of a geolocation.
%
...
days = size(result.smooth_plot,3);
post = result.smooth_plot(:,:,day);

% Map axes
imagesc(post), colormap(cmap), set(gca,'position',[0.2 0 0.8 0.8]); hold on
if ~isempty(mpt), plot(mpt.long_pix(1:day),mpt.lat_pix(1:day),'w'); plot(mpt.long_pix(day),mpt.lat_pix(day),'*w'); end
rel=plot(td.x0,td.y0,'^','markersize',10,'markerfacecolor','g','markeredgecolor','k');
rec=plot(td.x1,td.y1,'v','markersize',10,'markerfacecolor','r','markeredgecolor','k');
legend([rel rec],'Release pos.','Reported recapture pos.','location','southeast')
axis ij, axis(zm) , axis off,
